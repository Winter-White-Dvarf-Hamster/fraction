...
namespace Fraction.Test
{
    public class Tests
    {
        ...
        public void FractionParseCorrect(string process, string expected)
        {
            ...
            Assert.IsTrue(actual.ToString() == expected, $"Method does not work properly. Returned {actual} (Expected: {expected})");
        }
        ...
        public void FractionMultFractionCorrect(string left, string right, string expected)
        {
            ...
            var result = f1.Mult(f2);
            Assert.IsTrue(f1.Mult(f2).ToString() == expected, $"Method does not work properly. Returned {result} (Expected: {expected})");
        }
        ...
        [TestCase("8/4", 1, "1/1")]
        [TestCase("25/5", 1, "4/1")]
        [TestCase("22/11", 1, "1/1")]
        [TestCase("42/21", 1, "1/1")]
        public void FractionSubIntCorrect(string left, int right, string expected)
        {
            var f1 = Task4Kaba.Fraction.Parse(left);
            var f2 = new Task4Kaba.Fraction(right, 1);
            var result = f1.Sub(f2);
            Assert.IsTrue(f1.Sub(f2).ToString() == expected, $"Method does not work properly. Returned {result} (Expected: {expected})");
        }
        ...
        [TestCase("1/4", 3, "1/12")]
        [TestCase("1/5", 5, "1/25")]
        [TestCase("7/77", 7, "1/77")]
        [TestCase("21/21", 21, "1/21")]
        public void FractionDivIntCorrect(string left, int right, string expected)
        {
            var f1 = Task4Kaba.Fraction.Parse(left);
            var result = f1.Div(right);
            Assert.IsTrue(f1.Div(right).ToString() == expected, $"Method does not work properly. Returned {result} (Expected: {expected})");
        }
        ...
        {
            var f1 = Task4Kaba.Fraction.Parse(left);
            var result = f1.Mult(right);
            Assert.IsTrue(f1.Mult(right).ToString() == expected, $"Method does not work properly. Returned {result} (Expected: {expected})");
        }
        ...
        public void FractionAddCorrect(string left, string right, string expected)
        {
            ...
            Assert.IsTrue(f1.Add(f2).ToString() == expected, $"Method does not work properly. Returned {result} (Expected: {expected})");
        }
    }
}